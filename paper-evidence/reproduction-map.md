...
```bash
.venv/bin/python scripts/analyse_dc_doe_certificate.py
```

Inputs: `results/dc-*.json`. Expected: SPADE certifies with 66/66 containment; screened and
...
```bash
.venv/bin/python scripts/analyse_lc_confirmatory.py --glob 'results/lc-*.json'
```

Inputs: `results/lc-*.json`. Current output is R5 mean `+0.000855`, 95% CI
...
records that correction; `scripts/verify_conclusions.py` now guards the executable value.

### S1 — theta/tau mechanism

```bash
.venv/bin/python scripts/analyse_tt_theta_tau.py
```

Inputs: `results/tt-*.json`. Expected: TT-1 and TT-2 fail; volume difference `+0.000425` and
regret cost `+0.0301`.
...
```bash
.venv/bin/python scripts/run_real_ipsc_certification.py
.venv/bin/python scripts/calibrate_real_assay_loo.py
```
...
```bash
.venv/bin/python scripts/certify_hall_ogle.py
```

Inputs: canonical extractions under `data/published/`. Expected posterior-width ratio `297x`
and assay-specific LOO inflation `c=0.526`.

## Verification suite

```bash
.venv/bin/pytest tests/test_current_conclusion_guard.py tests/test_repository_audit.py -q
```
