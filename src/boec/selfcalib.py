...
__all__ = [
    "calibration_inflation", "calibration_tail", "conformal_lower_quantile",
    "conformal_lower_bound", "loo_residuals", "campaign_latent_inflation", "COND_MAX",
]
...
def campaign_latent_inflation(
    ...
    *,
    ...
    """Estimate a conservative, campaign-only latent-uncertainty multiplier.

    ``covariance`` must be the noise-inclusive fitted covariance at the observed wells.
    The multiplier is the upper tail of absolute LOO standardised residuals, clamped
    upward at ``minimum``.  This is deliberately an *opt-in diagnostic*: it is not a
    family-calibrated guarantee and callers must record the value in their protocol
    digest before using it to draw a certified region.
    """
    q = float(tail)
    floor = float(minimum)
    if not 0.0 <= q <= 1.0:
        raise ValueError(f"tail must be in [0, 1], got {tail}")
    if not np.isfinite(floor) or floor < 1.0:
        raise ValueError(f"minimum must be finite and >= 1, got {minimum}")
    mu, var = loo_residuals(covariance, y)
    sd = np.sqrt(var)
    estimate = calibration_tail(y, mu, sd, q=q)
    return max(floor, float(estimate))
...
def conformal_lower_quantile(y: ArrayLike, mu: ArrayLike, alpha: float = 0.10) -> float:
    ...
    ``y`` and ``mu`` must be predictions on a held-out calibration set.  The
    nonconformity score is ``mu - y``; the returned radius ``q`` yields the
    lower bound ``mu_new - q``.  The order-statistic definition (rather than
    an interpolated quantile) gives the standard ``1-alpha`` marginal
    coverage guarantee under exchangeability and is deterministic.
    """
    a = float(alpha)
    if not 0.0 < a < 1.0:
