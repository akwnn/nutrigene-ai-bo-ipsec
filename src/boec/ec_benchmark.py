...
def run_ec_replicate(family, *, config=ECBenchmarkConfig(), seed=0):
    ...
    return ECRow(family.name, seed, optimum - float(sp_pred.max().detach()),
                 optimum - float(do_pred.max().detach()), iou(sp_mask), iou(do_mask), rounds, 5)

def aggregate_ec(rows, *, bootstrap_replicates: int = 2000, bootstrap_seed: int = 0):
    ...
    gen = torch.Generator().manual_seed(int(bootstrap_seed))
    regret = torch.tensor([r.doe_regret - r.spade_regret for r in rows], dtype=torch.double)
    iou = torch.tensor([r.spade_region_iou - r.doe_region_iou for r in rows], dtype=torch.double)
    idx = torch.randint(len(rows), (bootstrap_replicates, len(rows)), generator=gen)
    regret_boot = regret[idx].mean(1)
    iou_boot = iou[idx].mean(1)
    def interval(x):
        q = torch.quantile(x, torch.tensor([0.025, 0.975], dtype=torch.double))
        return [float(q[0]), float(q[1])]
    return {'family':rows[0].family,'replicate_count':len(rows),'spade_regret_mean':mean('spade_regret'),'doe_regret_mean':mean('doe_regret'),'spade_region_iou_mean':mean('spade_region_iou'),'doe_region_iou_mean':mean('doe_region_iou'),'spade_rounds':mean('spade_rounds'),'doe_rounds':mean('doe_rounds'),'paired_regret_gain_doe_minus_spade_mean':float(regret.mean()),'paired_regret_gain_bootstrap_95ci':interval(regret_boot),'paired_iou_gain_spade_minus_doe_mean':float(iou.mean()),'paired_iou_gain_bootstrap_95ci':interval(iou_boot)}
