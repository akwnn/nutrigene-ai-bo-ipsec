...
The extractor that produced `data/external/hall_ogle_2025/*.json` validated itself by
...
def build_canonical_rows(tag: str, root: str | Path = ".") -> list[dict]:
    ...
    **`flagged` therefore marks RANK disagreement**: rows that are the maximum under one
    ...
    root = Path(root)
    rec = json.loads((root / f"data/external/hall_ogle_2025/{tag}.json").read_text())
