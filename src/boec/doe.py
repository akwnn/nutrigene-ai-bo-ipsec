...
class DoEResult:
    ...
    Attributes:
        ...
        Yvar_visited: ``(budget, 1)`` per-well observation variances returned by the
            evaluator, in the same measurement order as ``X_visited`` and ``Y_visited``.
...
def run_doe_arm(
    ...
    *,
    ...
    return DoEResult(
        ...
        Yvar_visited=torch.cat([Yvar1, Yvar2, Yvarc]),
...
def run_doe_unscreened_arm(
    ...
    *,
    ...
    return DoEResult(
        ...
        X_visited=X_all,
        Y_visited=torch.from_numpy(observed).reshape(-1, 1),
        Yvar_visited=torch.cat([Yvar_design, Yvarc]),
