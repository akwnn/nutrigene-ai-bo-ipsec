...
**Current EC-protocol status (1 September 2026).** The frozen six-factor, three-CQA benchmark has been rerun under the current checkout as a complete 384-row artifact (three families × 32 evaluation seeds × four arms), with a fail-closed analyzer verdict of `FAIL_CONTAINMENT`. The EC implementation suite passes 16 tests, and the publication validator passes all 9 guards. SPADE answers 17/32 broad campaigns (53.1%), 12/32 multimodal campaigns (37.5%), and 0/32 narrow campaigns. No false certificates were observed, but the one-sided 95% containment lower bounds are 0.816 and 0.758 for broad and multimodal. The current artifact and gate report are saved in `results/ec-evaluation-current.json` and `results/ec-evaluation-current-gate.json`. These results do not support a general claim that SPADE replaces DOE.

**Prospective extension decision.** The 48-well result remains immutable. A larger-budget study is valid only as a new protocol that registers exact schedules for SPADE, Sobol, qLogNEI, and both DOE comparators at each budget (for example 96, 192, 384, and 768 wells), with fresh training/holdout seeds and a new protocol digest. The DOE implementation rejects arbitrary budgets because its registered 48-well designs close at 20 + 27 + 1 (screen, response surface, confirmation); scaling SPADE alone would break the matched-budget comparison. Thresholds and the narrow-family landscape must not be changed post hoc.
...
**Revision status.** Search versus identification, qLogNEI co-primary, sequential RSM, multi-draw one-shot GP, Hartmann without forced 6→4, and selection-rule sensitivity are closed (Q54–Q59). The design-space programme (K6, P6–P8, Version C), all seven conditions of `spade-final-2026-08-23`, and the registered KF-3 mechanism follow-up are complete. The `doe_unscreened` comparator is implemented and has 100 campaigns in every six-factor condition; it is structurally unavailable at eight factors within 48 wells. Clean regeneration corrected stale pre-`dd75c91` feasibility flags in the stored checkpoints: the final prospective ledger now has five passes (KF-1, KF-6–KF-9) and five failures (KF-2–KF-5, KF-10). Both proposed repairs to second-plate targeting fail their registered bar, so the combined repair is moot. SPADE's certificate is measured on five families: no Hill cell is below nominal at 4,096 draws, it fails at γ = 0.99 on Levy and Rosenbrock, and it usually declines on Ackley and Hartmann6. The seven raw prospective artifacts and per-condition aliases are tracked, reproduce the 99,601-row study from a clean checkout, pass all nine release checks, and generate all eight publication figures. The retained threshold repair is a controlled-prevalence quantile; pooled-replicate noise estimation and strength-2 OA-LHS were tested at their registered gates and rejected. The stricter 48-evaluation joint SPADE protocol is implemented and frozen; its complete current-commit EC run is negative at the registered gate and is saved with its analyzer report. Remaining before submission: re-read citations from primary PDFs, write the manuscript, and retain the existing scope gaps—no d = 8 cost curves, no in-region long-run RSM recommendation, P7 Murphy is Hill d=6 σ=0.25 only, and no wet-lab validation.
...
| **KF-9** | all analysed primary cells are certifiable in principle | **PASS** — 0 of 23,600 primary-γ rows are at or above the certifiability ceiling after clean regeneration corrected stale checkpoint flags |
