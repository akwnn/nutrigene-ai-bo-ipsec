"""ACK -- does an attainable design threshold fix SPADE's ackley loss?

Implements docs/SPADE-ACKLEY-THETA-SPEC.md, frozen before this file was written.

`spade` reproduces LC bit-identically (explicit mu_max=1.0, the committed path).
`spade_adaptive` passes mu_max=None, so boec.multiround.resolve_theta takes theta from
the observed incumbent each round.
"""
...
P_GRID = (0.70, 0.30, 0.10)
CONFIGS = (("spade", 5), ("spade_adaptive", 5), ("qlognei", 5), ("qlognei", 10))
BUDGET, QBATCH = 48, 8
...
def lc():
    global _LC
    if _LC is None:
        _LC = _mod("_ack_lc", "run_lc_confirmatory.py")
    return _LC


def build(family, arm, seed, rounds):
    """LC's build, plus the spade_adaptive arm. mu_max=None -> adaptive theta."""
    L = lc()
    if arm != "spade_adaptive":
    ...
    n_init = BUDGET - QBATCH * (rounds - 1)
    ni, batches = round_schedule(BUDGET, rounds, n_init)
    return multiround_design(orc, P.DIM, seed, None, ni, batches, rho=0.95), orc
...
def main():
    ...
    ap.add_argument("--out", type=Path, default=ROOT / "results" / "ack-sweep.json")
        ...
        for arm, R in CONFIGS:
            try:
                ...
                with torch.no_grad():
                    ...
                    t_at_X = orc.truth(X).reshape(-1).double()
                    ...
                    for c in C_GRID:
                        ...
                        for p, tau in tb.items():
                            rows.append({"family": family, "seed": seed, "arm": arm,
                                         ...
                                         "inflation_c": float(c),
                                         "best_sampled": float(t_at_X.max()),
                                         "median_sampled": float(t_at_X.median()),
