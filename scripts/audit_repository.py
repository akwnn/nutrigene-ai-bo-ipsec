...
def lab_file_roles() -> dict[str, tuple[str, str]]:
    ...
    path = ROOT / "data" / "lab" / "overlay" / "bo_file_roles.csv"
    if not path.exists():
        return {}
...
def file_verdict(path: str) -> dict[str, str]:
    ...
    if path.startswith("data/lab/overlay/"):
        return _verdict(
            "SUPPORT", path, "Human-reviewed lab role, checksum, or gating overlay.",
    ...
    if path.startswith("data/lab/"):
        return _verdict(
            "SUPPORT", path, "Canonical lab manifest or derived input supporting real-cell analysis.",
    ...
    if path.startswith("data/published/"):
        return _verdict(
            "SUPPORT", path, "Canonical published-data extraction supporting independent real-data checks.",
    ...
    if path.startswith("data/oracles/"):
        return _verdict(
            "INFRASTRUCTURE", path, "Authoritative benchmark input used by cross-family evidence.",
    ...
    if path.startswith("results/") and name.startswith(CORE_RESULT_PREFIXES) and name.endswith(".json"):
        ...
        return _verdict(
            "CORE", path, rationale, claim=claims, role="canonical result",
    ...
    if path.startswith("results/") and any(token in name for token in ("real-ipsc", "published-ecm", "hall-ogle")):
        return _verdict(
            "SUPPORT", path, "Canonical real-cell supporting output.", claim="S2;S3",
    ...
    if path.startswith("scripts/") and (
        ...
        return _verdict(
            "INFRASTRUCTURE", path, "Active analyser, runner, or guard for a main paper claim.",
    ...
    if path.startswith("scripts/") and (
        ...
        return _verdict(
            "INFRASTRUCTURE", path, "Active real-cell supporting analysis.", claim="S2;S3",
    ...
    if path.startswith("tests/"):
        active = p.stem in ACTIVE_TEST_STEMS
        return _verdict(
            "INFRASTRUCTURE" if active else "ARCHIVE-VALID",
            path if active else f"archive/exploratory/{path}",
