...
def main():
    ...
    ap.add_argument("--historical", type=Path, default=None,
                    help="explicit directory containing the legacy constant-Yvar outputs")
    a = ap.parse_args()
    if not a.historical and not a.glob:
        raise SystemExit("refusing implicit DC input; pass --historical <directory> or --glob <pattern>")
    if a.historical:
        if not a.historical.is_dir(): raise SystemExit(f"historical directory not found: {a.historical}")
        paths = sorted(str(a.historical / f"dc-{f}.json") for f in FAMILIES
                       if (a.historical / f"dc-{f}.json").exists())
    else:
