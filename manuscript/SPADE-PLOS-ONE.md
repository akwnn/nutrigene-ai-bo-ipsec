...
When a cell-culture or bioprocess campaign has a fixed well budget, the end goal may be one best recipe or a trustworthy set of settings that still clear a threshold. Those are different jobs, so which experimental method looks best can change with the job. We compared SPADE (Synthetic Posterior Abstaining Design for Excursions)—a workflow built to return a conservative “good enough” operating region and to abstain when evidence is weak—with noisy Bayesian optimization (qLogNEI) and classical response-surface design of experiments (DoE) on shared synthetic test surfaces under a common 48-well budget. The main conclusion is decision-specific: on this benchmark SPADE is preferable when the deliverable is a trusted operating region, not when the deliverable is a single optimized recipe. At matched five feedback rounds, SPADE’s returned regions were larger on average than qLogNEI’s (mean certified volume 0.000864 versus 0.000009, about 92-fold; difference +0.000855, 95% CI [+0.000691, +0.001028], p < 0.000125; n = 320 paired contrasts). Certified volume is the fraction of a fixed 2,000-point scoring grid inside the region the method calls acceptable (one grid point equals 0.0005 of volume). At prevalence 0.30, every region SPADE returned contained truth (66/66 answered), whereas DoE answered more often but issued many incorrect regions (screened 85/122 contained among answers; unscreened 77/134); counting every campaign, contained totals were 66/160 for SPADE versus 85/160 and 77/160 for the DoE arms. That pattern fits an asymmetric loss in which a false certified region is costlier than abstention. By contrast, screened three-round DoE found better point recipes (+0.1026 SPADE-minus-DoE regret), and five-round SPADE showed no detectable regret difference from ten-round qLogNEI (−0.0005, 95% CI [−0.0221, +0.0207], p = 0.96; n = 160) without establishing equivalence. Results are limited to these synthetic surfaces, moderate noise, and finite grids; they do not establish wet-lab or regulatory performance.
