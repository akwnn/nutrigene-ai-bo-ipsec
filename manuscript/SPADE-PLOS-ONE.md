...
When a cell-culture or bioprocess campaign has a fixed well budget, the end goal may be one best recipe or a trustworthy set of settings that still clear a threshold. Those are different jobs, so which method looks best can change with the job. We compared SPADE (Synthetic Posterior Abstaining Design for Excursions)—built to return a conservative operating region and to abstain when unsure—with noisy Bayesian optimization (qLogNEI) and classical response-surface DoE on shared synthetic surfaces under a common 48-well budget. The main conclusion is decision-specific: on this benchmark SPADE is preferable for a trusted operating region, not for a single optimized recipe. At matched five rounds, the volume contrast is driven mainly by how often each method returns any region at all: SPADE answered 116/320 contrasts versus 6/320 for qLogNEI. Mean certified volume across all contrasts, with abstentions scored as zero, was 0.000864 versus 0.0000094 (difference +0.000855, 95% CI +0.000691 to +0.001028, p < 0.000125). Certified volume is the fraction of a 2,000-point grid inside the returned region (one point = 0.0005). Conditional on answering, mean volumes were 0.00238 (SPADE) and 0.00050 (qLogNEI). At prevalence 0.30 versus DoE, SPADE contained truth in all answered regions (66/66) but abstained more; DoE answered more often with many incorrect regions. Screened three-round DoE found better point recipes (+0.1026 SPADE-minus-DoE regret), and five-round SPADE showed no detectable regret gap versus ten-round qLogNEI (−0.0005, 95% CI −0.0221 to +0.0207, p = 0.96; n = 160) without proving equivalence. Results are limited to these synthetic surfaces, moderate noise, and finite grids—not wet-lab or regulatory performance.
## Introduction
...
Table 1 states the local certification dimensions for C3. Under registered leave-one-family-out calibration at matched five rounds, qLogNEI almost never returned a non-empty conservative set (6/320 contrasts; mean certified volume 0.0000094 ≈ 0.019 grid points per contrast), whereas SPADE returned a non-empty set in 116/320 contrasts (mean 0.000864 ≈ 1.73 grid points). Abstentions are scored as zero volume, so these means are unconditional. The SPADE-minus-qLogNEI difference was +0.000855 (95% CI +0.000691 to +0.001028, p < 0.000125; n = 320 paired contrasts). The volume contrast is therefore driven primarily by answer rate rather than by larger regions among answered campaigns: conditional on answering, mean volumes were 0.00238 for SPADE and 0.00050 for qLogNEI (one grid point). Certified volume is the fraction of the 2,000-point scoring grid inside the returned region (one point = 0.0005). No smallest effect of interest was registered for volume; we treat the contrast as directional support for the tested operating-region decision, not universal superiority. The analogous positive three-round claim was withdrawn after extension to 32 seeds.

![SPADE operating-region advantage.](../results/paper-figures/plos/fig2.png)

**Fig 2. SPADE’s primary regional advantage versus qLogNEI.** (A) Certified-volume difference at matched five rounds (+0.000855, 95% CI +0.000691 to +0.001028, p < 0.000125; n = 320), with abstentions scored as zero. SPADE answered 116/320 contrasts versus 6/320 for qLogNEI. (B) Versus DoE at prevalence 0.30: containment among answered regions (SPADE 66/66; screened DoE 85/122; unscreened DoE 77/134). Unconditional contained counts are 66/160, 85/160, and 77/160. (A) Certified-volume difference (SPADE minus qLogNEI) at matched five rounds: +0.000855 (95% CI [+0.000691, +0.001028], p < 0.000125, n = 320). Absolute means were 0.000864 (SPADE) and 0.000009 (qLogNEI) on the 2,000-point grid (one point = 0.0005 of volume). (B) Observed truth containment among answered regions at prevalence 0.30: SPADE 66/66 versus screened DoE 85/122 and unscreened DoE 77/134. Unconditional contained counts are 66/160, 85/160, and 77/160.
