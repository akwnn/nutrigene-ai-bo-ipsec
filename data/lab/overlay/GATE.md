...
Everything below was produced by `python scripts/build_lab_dataset.py` and lands in
...
`tests/test_lab_gating.py::test_median_comparison_would_have_picked_the_wrong_channel`
...
6. Set `status` to `gated`, and update
   `tests/test_lab_manifest.py` / any test asserting `y` is empty **in the same commit**.
...
- Do not paste `y_candidate` into `y` without doing §4. The column names differ on
  purpose, and `tests/test_lab_dataset.py` enforces that no derived table ships a
  column called `y`.
