...
| Box and Wilson (1951); Box and Draper; Myers and colleagues | Classical experimental-design background | Sequential response-surface methodology and steepest ascent; canonical classification of maxima, minima, and saddles; separate constrained/ridge analysis; and the principle that design and model interpretation are inseparable | Superiority of the specific classical arm used here |
...
| Rummukainen and colleagues (2024) | Closest matched-count executed precedent | A 15-run Box--Behnken design and a BO sequence with five shared initial experiments and ten new experiments; noisy EI chose the first nine new experiments and posterior mean the tenth | A common post-campaign terminal comparison or a universal conclusion that BO cannot save experiments |
...
| Lapierre and colleagues (2025) | Executed multicycle media optimization | Batch BO compared with a two-step DoE workflow after shared screening in microbial media optimization | A CCD label, an exactly 48-condition shared screen, or isolation of sampling, surrogate, and terminal-rule effects |
| Ndahiro and colleagues (2025) | Constraint-aware mammalian-media optimization | Integration of Bayesian optimization and thermodynamic constraints, compared with an equal-count JMP space-filling design in a wet-lab setting | A comparison with classical RSM or representation of solution thermodynamics in this unconstrained synthetic benchmark |
...
Bibliographic metadata and the claims above were rechecked against primary full texts on 2026-08-26. The paper should not use a review when the original experimental article is available, and it should not imply that Hall et al.'s experimental values generated the synthetic response surfaces.
