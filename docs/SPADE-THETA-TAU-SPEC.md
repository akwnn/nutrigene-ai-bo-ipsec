...
- **FAIL** -> targeting does not help even when correctly aimed. The two-plate and
  multi-round lines would then agree, and the paper would say plainly that certificate-
  contour targeting does not earn its complexity. **This is a real possible outcome and
  is registered first.**
...
## 8. TT RESULT — bounded and not adoptable
...
### 8.1 TT-1 (PRIMARY) is inconclusive
...
Per family, the aggregate null is compatible with heterogeneous effects. These family
estimates are descriptive only; after Holm correction across the five family tests, the
two smallest p-values (0.012 and 0.015) are both 0.06, so no family result is confirmatory
...
Correct targeting helps hartmann6 and hurts ackley by similar magnitudes, and does nothing
on the three families where theta and tau already nearly coincided. There is no consistent
benefit, but the aggregate interval includes zero and does not justify a clean failure claim.
...
the non-inferiority guardrail fails, although the interval's lower endpoint is below the
registered SESOI of 0.02. Mechanically unsurprising: tau at p=0.30 is a 30th-percentile threshold, so
...
This is the paper's bounded result about the method. SPADE's advantage does not come from
