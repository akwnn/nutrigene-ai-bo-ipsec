...
Observations are noisy. The simulator uses relative noise `σ ∈ {0.10, 0.25}` together with a small additive term, as specified in `docs/METHODS.md`. The registered inferential sample is 25 landscapes. Each arm is run with two random seeds per landscape and the two runs are averaged, so inference is over landscape means rather than nominal runs. Two contrasts were additionally powered to 100 landscapes as a declared post-hoc extension (Q67, Q68). Because `generate_ensemble` accepts landscapes sequentially from `seed0`, the registered 25 are an exact prefix of the 100 — verified by instance id and seed — so the extension adds landscapes rather than replacing them, and both are reported.
...
Contrasts are always reported as DoE/RSM minus BO on paired landscape means, so negative values favour DoE/RSM. Uncertainty is summarized with paired bootstrap 95% intervals over landscape means (25 registered; 100 where a power extension is reported). Wilcoxon signed-rank tests on the same paired differences govern directional declarations. Equivalence is assessed by two one-sided tests with SESOI `0.02`. An interval covering zero is not treated as equivalence.
...
Restricting the fitted models to the region actually sampled brings the methods back together. In the primary cell the classical quadratic gives **0.1169**, the GP gives **0.1232**, and the contrast DoE − GP is **−0.0063** [−0.0233, +0.0107]. This is inconclusive at `n = 25`: the mean lies well inside the equivalence margin, but the minimum detectable effect is about `0.027`, larger than the prespecified SESOI `0.02`. The cell was underpowered to test the effect it was built to test.

A declared power extension to `n = 100` resolves it (Q68, `results/q68-inregion-n100.json`). **These rows are a fresh post-fix regeneration, not a rescore of the stored pre-fix campaigns above, and the two must not be read as the same numbers** — see the reproducibility note in Methods. On the same 25 landscapes the regenerated contrast is **−0.0055** [−0.0215, +0.0119], consistent in sign and magnitude with the stored **−0.0063**. Extended to 100 landscapes:

| Contrast | Mean | 95% CI | Wilcoxon | TOST |
|---|---:|---|---:|---|
| Classical quadratic − qLogEI GP | **−0.0023** | [−0.0100, +0.0057] | 0.675 | **equivalent** |
| Classical quadratic − qLogNEI GP | **−0.0006** | [−0.0098, +0.0082] | 0.929 | **equivalent** |

Under the fair in-region readout at matched budget, the two approaches are equivalent within the smallest effect this study declared interesting. The other two rules sharpen rather than move at `n = 100`: measured-value argmax **−0.0668** [−0.0754, −0.0582] and unconstrained model recommendation **+0.2951** [+0.2813, +0.3082]. At `d = 8`, `σ = 0.10`, the in-region contrast is **+0.0001** [−0.0118, +0.0115] and is equivalent within `0.02` at `n = 25`.

The stored GP in-region values come from the same fitted BO GP as the unconstrained rule because the GP optimum lies inside the sampled region. That was previously a manuscript assertion with no per-campaign record behind it; it is now measured. Q64 (`results/q64-gp-inregion.json`) computes the region-constrained peak explicitly from the bounding box of the visited wells and finds containment in **100 of 100** primary-cell campaigns across both acquisitions, with box and region regret differing by at most `1.5e-06`. Q68 reproduces the same 100% containment at `n = 100`. The assertion held. Coverage is the primary cell only; the `d = 8` and `σ = 0.10` cells are not yet recorded, and the figure code falls back to the earlier factorial values for any cell Q64 does not cover.
...
At 48 wells, DoE/RSM uses **3** plate rounds, qLogEI and qLogNEI use **10**, and the one-shot GP arm uses **1**. The long-budget arrival analysis to 200 wells shows why that distinction matters. Relocating sequential RSM (`doe_ascent`) removes the easy headline that BO is more well-efficient than “the classical arm,” because repeating an in-place CCD is a control rather than textbook sequential RSM. At lower noise and target `0.10`, relocating RSM reaches the target on **16/25** landscapes, compared with **13/25** for in-place CCD. Paired well-count ratios where defined lie between **0.73** and **1.02**. Two multiplicity corrections are reported and they disagree, so both are stated. No rule-A arrival contrast survives Holm over the registered family of **26**. Within the rule-A sub-family of **ten**, one cell survives: at `σ = 0.10` and target `0.05`, qLogEI reaches the target on **18/25** landscapes against relocating RSM's **6/25** (exact McNemar `0.0042`; adjusted `0.042` over ten, `0.109` over twenty-six). The next-smallest cell, `σ = 0.10` at target `0.10`, fails under either correction (`0.0078` exact; `0.070` over ten, `0.094` over twenty-six). We therefore make no general well-count efficiency claim: the single surviving cell is narrow, it does not survive the full family, and at looser targets every arm arrives — random included.
...
Several limitations remain. The oracle family is synthetic and relatively smooth, with additive share near 0.93. The registered inferential sample is 25 landscapes; only the primary in-region contrast and the low-noise posterior-mean contrast have been extended to 100. GP-based recommendation rules are conditional on a miscalibrated surrogate, with worst latent 95% coverage of **76.4%** — which is also the most plausible reason posterior-mean selection helps at `σ = 0.10` (Q67: **+0.0146** [+0.0084, +0.0210] at `n = 100`, favouring BO, and itself inside the equivalence margin) while doing nothing at `σ = 0.25`. Q60 and Q61 remain blocked by replay and shadow failures in the current BO path, so this draft does not answer whether averaged confirmation or `q = 1` sequencing would change the primary measured-value result.

Most importantly, **stored BO results in this project cannot be reproduced from code and seed.** Replaying a committed BO file at its own commit, in a byte-identical environment, returns some rows bit-exact and others different by up to `1.2e-01`, because the acquisition optimizer's retry depended on per-process warning state under sharded execution. The classical arm is unaffected and replays bit-exactly, so the comparator is sound; the BO side is not. A reseed of the acquisition call fixes this going forward, which also means post-fix runs do not reproduce pre-fix stored values. Numbers in this draft are pre-fix unless explicitly labelled as a regeneration. No wet-lab validation is claimed.
