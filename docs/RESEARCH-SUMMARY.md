...
TruVaR's prior unification of optimization and level-set estimation. A targeted
comparison table includes physical studies and a 2026 synthetic batch-BO benchmark;
it is not a systematic review or a cross-paper performance ranking. SPADE has not
been compared directly with specialized level-set methods. Against one-shot LHS,
primary SPADE's mean map advantage is only 0.0063, with worse mean point regret and
an additional feedback round; no retained paired claim decision establishes LHS
equivalence or SPADE superiority. Its paired map advantage over Sobol is entirely
below the 0.02 practical margin. These contrasts qualify the larger 15.3% gain
against qLogNEI. Target-condition paired inference uses 25 landscape instances,
not 100 independent landscapes or 99,600 independent experiments. The contribution
is the controlled evaluation and decision audit, not a new acquisition principle,
biological validation, or universally preferable method. See `docs/source_verification.md`
for primary-source checks. No new scientific campaign was run for this revision.

**Publication interpretation correction (2026-09-09):** the original prospective map
mask uses a fixed 0.50 probability cutoff, regardless of stored gamma/alpha labels.
The original `containment_cell` analyzer counts held-out posterior probabilities
meeting alpha, not oracle-containment indicators. Its Hill certificate results are
model self-consistency checks and cannot establish empirical validity, even in Hill.
The manuscript and current Figure 4B disclose this. Accordingly, prospective results
throughout this file are described as posterior self-consistency checks, not empirical
certificate validity. Separate retrospective oracle-containment evidence and the later
joint-protocol `NO_SELECTION` result remain distinct. No primary result files were altered.

**Terminal-rule source correction (2026-09-09):** manuscript Figure 2 reads
`fix1-terminal-rule.json`, whose generating script fits a common GP to every arm,
including classical designs, and locates its posterior-mean maximum using a
20,000-point screen, 20 restarts, and 4,096 raw starts. The earlier arm-specific
quadratic extrapolation diagnostics are a separate comparison, not the explanation
of Figure 2's Rule-P contrast. The manuscript now distinguishes them. A fresh exact
replay of one classical Figure 2 source campaign differed by 3.88e-7; the aggregate
was preserved and the limitation disclosed. The calibration checkpoint writes
successfully before an intentional fidelity-gate stop, not a checkpoint-write crash.
Older narrative below must be read subject to these source corrections.

The main publication evidence body remains the completed prospective study
`spade-final-2026-08-23`: 99,601 combined records across seven conditions, nine release
checks passed, and four current main figures generated. The count comprises 99,600 scored
configurations from 8,300 campaign-arm executions evaluated under 12 stored configurations,
plus one unavailable-design declaration; it is not 99,600 independent experiments. Its strongest contribution is the
controlled demonstration that terminal decision rules and scored objects can change
BO-versus-RSM rankings. Historical SPADE variants improve design-space mapping in the
prespecified Hill target, but their targeting mechanism and certificate portability are
bounded by the negative results summarized below.

The newer 48-evaluation joint protocol is a separate evidence body. On 2026-08-31, its
complete five-family development grid was reconciled at 550 rows per family (2,750 total).
The registered unanimous leave-one-family-out selector returned `NO_SELECTION`: every
candidate missed the prespecified 0.90 empirical-containment floor on at least one family.
No power artifact was authorized and the lockbox remains `FROZEN_UNOPENED`. The manuscript
...
**Results.** On the same campaigns, rankings reversed. At σ = 0.25, measured-value argmax favoured sequential DoE by 0.0595 (d = 6) and 0.0284 (d = 8); the primary contrast against qLogNEI was −0.0574. About 55–73% of that lead was identification rather than search, and confirming the top three wells reduced the primary contrast to −0.0009 (null). Naïve unconstrained recommendation favoured BO by 0.27–0.36 because the quadratic was a saddle in 200/200 Hill runs; in-region recommendation was null in three of four cells. Design-space scoring reordered the arms: `doe` was last on map quality in 23 of 24 Hill cells and symmetric-difference-worst in 36 of 92 cross-family cells. Historical SPADE variants led the Hill map range and retrospective refinement ranking, with mid-field calibration and practical regret parity. Separate retrospective oracle-containment evidence failed at γ = 0.99 on Levy and Rosenbrock and usually declined on Ackley and Hartmann6. The prospective Hill statistic measured posterior self-consistency, not oracle containment, so it established no empirical certificate validity. The prospective artifact contains 99,600 scored configurations over 8,300 campaign-arm executions plus one unavailable-design declaration across seven conditions. Boundary targeting did not beat random placement (−0.0019, p = 0.41), and its +0.0117 gain over one plate was below the 0.02 SESOI. A 400-campaign follow-up found that two targeting repairs also missed that bar.

**Historical manufacturing extension and the 90% safety benchmark.** A separate synthetic multi-CQA qualification benchmark intersects attribute-specific certificates, reports the limiting attribute, and abstains when the joint region is unsupported. In 150 paired evaluations, joint SPADE had 100% oracle containment in aligned and moderate-conflict families—above that benchmark's 90% target—while scalar-only certification had 52% and 0% containment. Under strong conflict, joint SPADE returned no region; scalar-only certification returned a region with a median unsafe fraction of 100%. This historical synthetic result is neither prospective lockbox evidence nor biological validation, and it does not override the later joint protocol's `NO_SELECTION` result or authorize opening its lockbox.

**EC-differentiation replacement benchmark.** A separate registered six-factor EC-style benchmark compares SPADE (48 wells; 32 + 8 + 8; three rounds) with sequential DOE/RSM (48 wells; 32 + 4 + 4 + 4 + 4; five rounds) on broad, narrow, and multimodal synthetic differentiation landscapes. Across 25 replicates per family, SPADE recovered the acceptable region better in every family (IoU 0.853 vs 0.840, 0.843 vs 0.823, and 0.719 vs 0.610). Point performance was comparable: DOE was marginally better on broad-family regret, SPADE was better on narrow-family regret, and SPADE had a small disadvantage on the multimodal family; paired bootstrap intervals included zero for all regret contrasts. The multimodal region-recovery gain was +0.110 (95% bootstrap CI [+0.074, +0.148]). The result supports a bounded claim that SPADE improves region recovery at matched well cost and reaches its terminal decision in fewer adaptive rounds; it does not establish universal superiority or biological efficacy.

**Joseph evidence integration (2026-08-29).** The final LC/TAU/ACK adjudicators and their committed result packages are now present locally as a selective port. LC reports the registered all-family regret contrast as inconclusive at the SESOI boundary, while its leave-one-seed-out certification pattern supports the bounded five-round SPADE versus ten-round qLogNEI result. TAU-1 passes (Spearman ρ = 0.9801) and TAU-2 shows Hill can certify at prevalence 0.70, but neither licenses a universal claim. ACK-1 fails: the proposed fixed-θ Ackley explanation is withdrawn; adaptive θ remains research-only and is not enabled in the registered product arm. No registered B5 digest or active Cursor worktree was rewritten.
...
**Revision status.** Historical Q54–Q59, K6/P6–P8/Version C, all seven prospective conditions, and the KF-3 follow-up are complete. The prospective ledger has five passes (KF-1, KF-6–KF-9) and five failures (KF-2–KF-5, KF-10), but KF-1 is only a posterior model-check pass and is not empirical validity. Separate retrospective oracle-containment results show no sub-nominal Hill cell at 4,096 draws, γ = 0.99 failures on Levy and Rosenbrock, and frequent refusal on Ackley and Hartmann6. The seven raw prospective artifacts reproduce 99,600 scored configurations from 8,300 campaign-arm executions plus one unavailable-design declaration and pass all nine prospective release checks. The current submission has four main figures; older exports are historical diagnostics. The prospective maps use a fixed 0.50 cutoff. The separate 2,750-row joint protocol returned `NO_SELECTION`; no power artifact was created and the lockbox remains unopened. Submission still requires author-controlled declarations and approvals, resolution or acceptance of disclosed historical replay limitations, and a clean DOI-bearing archival snapshot. No wet-lab validation exists.
...
The historical interactive diagnostics show winner reversal by terminal rule, cost in wells and rounds, and the saddle/ridge mechanism. They are not the current main-figure inventory. The current manuscript's Figure 2 instead compares terminal rules after fitting the same GP recommender to every arm (20,000 screen points, 20 restarts, and 4,096 raw starts). The factorial (Q34/Q45) and protocol sensitivities (Q54–Q59) remain historical supporting analyses.
...
Lower regret is better. The three items below are historical interactive diagnostics and
are preserved as scientific context; they are not the current manuscript's main figures.
The submission authority is the four-figure inventory in `manuscript/SPADE-PLOS-ONE.md`:
Figure 1 defines benchmark decisions and estimands, Figure 2 reports terminal-rule
sensitivity under a common GP recommender, Figure 3 reports point, map, and round evidence,
and Figure 4 separates calibration, posterior checks, and empirical containment.
Control-arm tables, the 2 × 2 factorial grid, and the Q42 family tables are historical
supporting analyses.

- **Historical diagnostic 1** (`results/figures/fig1-scoring.html`): DoE versus BO at 48 wells, by scoring rule and noise.
- **Historical diagnostic 2** (`results/figures/cost-curves.html`): regret versus wells and versus rounds, to 200.
- **Historical diagnostic 3** (`results/figures/fig3-saddle.html`): why the quadratic is a saddle and why ridge constraints matter.

### 5.1 Matched budget N = 48 (historical scoring diagnostic)
...
**Historical diagnostic figures.** The stored interactive scoring graphic leaves unmeasured cells blank and checks winner reversal; the separate cost graphic leads with P(T ≤ N). Neither is the current manuscript's Figure 1 or Figure 2.
...
**What a laboratory would actually choose on.** Mean regret and mean rank both compress SPADE to "middling"; the performance profile shows what kind. **SPADE is within 3× of the best method on 70% of problems and wins outright on 2%. `doe` shares the 2% win rate, is within 3× on only 22%, and fails to reach even 10× on 42%.** Source: `results/versionc-conventions.json` (σ=0.10, rule P; FINDINGS-SPADE.md §42.2). For a laboratory committing one protocol to one plate, that tail is the decision, and no mean-based metric in this study reports it.
...
**The prospective confirmatory study narrows the method claim.** Everything above is a re-score of stored campaigns. A separate registered study (`spade-final-2026-08-23`) runs SPADE **as a method** — three SPADE arms, causal controls, a pre-run regime classifier, seven conditions and a ten-item kill ledger committed before the result files existed. The completed merge contains **99,601 records**: 92,400 original rows, 7,200 `doe_unscreened` rows at the six-factor conditions, and one structured declaration that an eight-factor unscreened second-order design is infeasible within 48 wells. The 99,600 scored configurations arise from 8,300 campaign-arm executions scored under 12 configurations; they are not independent experiments. The original maps use a fixed 0.50 probability cutoff regardless of stored gamma labels. Five ledger checks pass and five fail, but the certificate entries below are posterior model checks rather than empirical validity tests:

| kill | claim under test | final verdict |
|---|---|---|
| **KF-1** | held-out posterior self-consistency under cross-fit | **PASS as a model check only** — no eligible cell was demonstrably below its posterior comparison level; this does not measure oracle containment or establish empirical validity, even in Hill |
| **KF-2** | posterior self-consistency extends beyond Hill | **FAIL** — the Hartmann confirmatory family is not uniformly clean; this result cannot support a cross-family validity claim |
...
| **KF-9** | feasibility-ceiling screen | **PASS** — 0 of 23,600 primary-label rows entered analysis at or above the registered ceiling; this audit guard is not evidence of certificate validity |
| **KF-10** | empty-set degeneracy does not explain a pass | **FAIL** — 18 of 64 raw-PASS cells certify nothing in more than half their campaigns and are downgraded to inconclusive |
...
**Implementation scope.** These verdicts apply to the SPADE implementation that exists, not every stage in its founding specification. The controlled-prevalence threshold correction was retained after a 400-campaign follow-up improved answerability on Ackley and Hartmann6, although the smallest target regions remained effectively unanswerable. Plate 1 remains plain LHS: a 125-campaign test found that exact strength-2 OA-LHS at 49 wells did not reduce the Hartmann6 design lottery relative to 48-well plain LHS. The replicate-pooled noise estimator was not built because even oracle knowledge of the true noise variance improved mean regret by only 0.00816, below the registered 0.01 build bar. Stage 0 covariate adjustment remains out of scope because the required data do not exist. The new frozen 48-evaluation joint protocol carries these decisions forward: a scrambled-Sobol opening, learned-noise GP, and a threshold defined as a quantile of the γ-adjusted reliability margin.

**Scope.** The re-scored programme, the prospective seven-condition study and the KF-3 follow-up are separate evidence bodies and must not be pooled. The frozen regime detector failed on both held-out families (0 of 50 each), so that study has no working automatic landscape-scope gate. `doe_unscreened` is implemented and run at every six-factor condition; at eight factors it is structurally unavailable because a replicated full second-order CCD cannot fit within 48 wells. The seven raw condition artifacts, their per-condition aliases, and the corrected derived analyses are tracked. Historical exports remain available, but the manuscript and its four current main figure exports are the submission authorities. Clean-checkout validation passes 9/9 prospective release checks; those checks do not resolve the disclosed historical exact-replay failures.
...
The historical diagnostics show winner reversal on one synthetic ensemble, cost in wells and rounds, and why the unconstrained quadratic is a saddle. They remain supporting context, not the current Figure 1–3. In the current submission, Figure 2 applies the common GP recommender to every arm; the earlier quadratic extrapolation result is a separate diagnostic. Q34/Q45 show that sampling geometry and surrogate class are separately measurable, while the CCD remains more D-efficient in its own region.
...
1. **Need a map of a planned region, or may change the criterion later.** Run classical DoE (screen + CCD). That is what the CCD is D-efficient for. Do not read the unconstrained polynomial peak as the answer; it was a saddle in 200/200 Hill runs (historical saddle/ridge diagnostic). Use in-region / ridge.
...
Q54–Q59 closed the registered historical revision workstreams (Section 5.8). Remaining historical limits include: the long-run `doe_ascent` model recommendation is not an in-region ridge recommendation; Q58's tie uses confirmation alone rather than an average of original and confirmation readings; cost curves were not computed at d = 8; and d = 8 Hartmann cannot be unscreened inside 48 wells. The old interactive saddle graphic is historical and is not part of the current four-main-figure submission set. Q49's 61% identification share is LHS at n = 192, not E2. Coordinate descent is unpaired. No wet-lab validation exists.

**Design-space limitations (Section 5.9).** The re-scored programme, the prospective study `spade-final-2026-08-23` and the registered KF-3 follow-up are separate evidence bodies and must not be quoted as one. Five of ten prospective ledger checks fail; the registered consequence is a narrowed claim, not suppression. The consequential mechanism result survives two attempted repairs: original straddle targeting, error-volume-aware acquisition and diversity-aware batching all fail to beat random placement by the registered 0.02 margin at the Hill target. The repairs show smaller, statistically non-null Hartmann gains, but still miss that practical bar; the combined repair is therefore moot. The `doe_unscreened` comparator is implemented at every six-factor condition, closing the earlier NOT_RUN gap. Prospective `containment_cell` values are posterior self-consistency checks and establish no empirical certificate validity. Separate retrospective oracle-containment evidence remains family-dependent: at 4,096 draws no Hill cell is below nominal, Levy and Rosenbrock fail at γ = 0.99 in three of 64 scored cells, and Ackley and Hartmann6 usually decline to certify. SPADE is mid-field on calibration while first on refinement, so its sharpness claim is better supported than its reliability claim. The prospective maps use a fixed 0.50 cutoff. The controlled-prevalence threshold correction is retained; OA-LHS and replicate-pooled noise estimation were rejected by registered empirical build gates rather than left as unexplored remedies. Stage 0 remains unmeasured, and a d = 8 unscreened CCD is arithmetically impossible inside 48 wells. The restored raw condition artifacts pass their nine release checks, but historical exact replay remains disclosed and unresolved.
...
9. The original prospective posterior checks do not establish empirical certificate validity, even in Hill. Separate retrospective five-family oracle-containment evidence finds γ = 0.99 under-coverage on Levy/Rosenbrock and frequent refusal to certify on Ackley/Hartmann6. The later future-response protocol returned `NO_SELECTION`; no lockbox evidence was opened.
...
| Historical cost-curve diagnostic | `results/figures/cost-curves.html` |
| Historical scoring-rule diagnostic | `results/figures/fig1-scoring.html` |
| Historical saddle / ridge diagnostic | `results/figures/fig3-saddle.html` |
| Current four main figure exports | `results/paper-figures/plos/fig1.png` through `fig4.png` |
