...
**Revision status.** Search versus identification, qLogNEI co-primary, sequential RSM, multi-draw one-shot GP, Hartmann without forced 6→4, and selection-rule sensitivity are closed (Q54–Q59). The design-space programme (K6, P6–P8, Version C) and the prospective study `spade-final-2026-08-23` (all seven conditions) are complete and reported in Section 5.9. Outstanding: the `doe_unscreened` comparator is unimplemented, leaving one registered kill NOT_RUN. SPADE's certificate is now measured on five families: it holds on Hill, fails at γ = 0.99 on levy and rosenbrock, and is declined on ackley and hartmann6. Remaining: unconstrained long-run “model recommendation” is still not in-region ridge; cost curves were not computed at d = 8; citations should be re-read from PDFs; no wet-lab validation.
...
**The prospective confirmatory study narrows the method claim.** Everything above is a re-score of stored campaigns. A separate registered study (`spade-final-2026-08-23`) runs SPADE **as a method** — three arms, a pre-run regime classifier, a frozen local rule, seven conditions, 92,400 rows, with the spec, the arms, the numeric bars and a ten-item kill ledger all committed before any runner file existed. Its registered rule is that the broad SPADE-method paper is supportable only if all seven conditions hold, and that **if any fails the claim narrows and is never hidden.** Four failed:

| kill | claim under test | verdict |
|---|---|---|
| **KF-3** | targeted plate 2 earns its complexity | **FAIL** — `m0` does not beat *random* plate-2 placement: **−0.0019** [−0.0062, +0.0026], p = 0.41 |
| **KF-4** | plate 2 does anything at all | **FAIL** — it does beat plate-1-only, **+0.0117** [+0.0074, +0.0156], p_adj = 1.3e-04, but that is **below the 0.02 SESOI** |
| **KF-5** | m > 0 lowers regret safely | **FAIL** — **−0.0028** [−0.0076, +0.0021], p = 0.31: a trade-off, not an improvement |
| **KF-10** | empty-set degeneracy does not explain a pass | **FAIL** — **18 of 64** passing cells have an empty rate above 50% |
| KF-1 / KF-2 | certificate valid prospectively / beyond hill | INCONCLUSIVE / NOT_RUN |
| KF-6–KF-9 | — | PASS |

KF-3 is the consequential one. **Stage 5's boundary-targeted second plate — the step that distinguishes SPADE from any two-round spread design — does not outperform placing the same wells at random.** The second plate helps, but by less than the smallest effect declared worth having, and its *targeting* is not what makes it help. The registered consequence applies: the method claim narrows from "SPADE works" to "a second plate helps slightly, and the mechanism that was supposed to make it help is unsupported."

One condition is genuinely unresolved rather than failed: `doe_unscreened` is spec-mandatory and arithmetically feasible at d = 6 (28 coefficients against 48 wells) but was never implemented, so KF-2 is **NOT_RUN — which the ledger's own semantics record as "not a pass."** Four validator violations, one per primary condition, all name it.

**Scope.** The re-scored results and the prospective study must not be quoted as one body of evidence. Its regime detector was frozen before scoring and **failed** — no first-plate statistic separated smooth from deceptive landscapes on either held-out family (0 of 50 on both), so the protocol ships without that stage. Eight registered kill conditions are adjudicated and none fired; three are moot because the trust-region form was never built, and two pass structurally rather than evidentially because a re-score reproduces its source by construction. A d = 8 unscreened comparison is arithmetically impossible at 48 wells — 45 parameters, no central composite design fits — so that confound is permanent.

Sources: `results/k6-analysis.json`, `results/p6-families/` (8 shards + `results/p6-families.meta.json`, read via `scripts/load_p6_families.py`), `results/p7-murphy.json`, `results/p8-certificate-families.json`, `results/f3-draw-sweep.json`, `results/q59-map-rescore.json`, `results/versionc-kills-s010.json`, `results/versionc-kills-s025.json`; protocol `docs/SPADE-SPEC.md`; full record `docs/FINDINGS-SPADE.md`; registrations `docs/OPEN-QUESTIONS.md`.
...
**Design-space limitations (Section 5.9).** The re-scored programme and the prospective study `spade-final-2026-08-23` are separate bodies of evidence and must not be quoted as one. In the prospective study four of ten registered kill conditions failed, and the registered consequence — that the method claim narrows rather than being withheld — applies. The most consequential is that Stage 5's boundary-targeted second plate does not outperform random placement of the same wells, so the mechanism that distinguishes SPADE from any two-round spread design is unsupported. One mandatory comparator, `doe_unscreened`, is arithmetically feasible at d = 6 but was never implemented, leaving KF-2 NOT_RUN — which the ledger's own semantics record as not a pass. Its certificate fails below nominal at γ = 0.99 on levy and rosenbrock (three of 64 cells, Holm-significant across all 64), and ackley and hartmann6 decline to certify at all, leaving Hill as the only family that is both willing and calibrated — and Hill is this study's own benchmark. SPADE is mid-field on calibration while first on refinement, so its sharpness claim is better supported than its reliability claim. The regime detector that was to identify deceptive landscapes from plate 1 was frozen before scoring and failed on both held-out families (0 of 50 each), so the protocol ships without it and landscape class remains undetectable by the one statistic tested. Two of the eight adjudicated kill conditions pass structurally rather than evidentially, because a re-score reproduces its source by construction, and the registered regret bar mixes estimands so the like-for-like verdict is parity rather than a win. Stage 0's covariate R² is not recoverable from existing data, so the variance-reduction stage is specified but unmeasured. A d = 8 unscreened comparison is arithmetically impossible at 48 wells (45 parameters, no central composite design fits), so that confound is permanent and no future run removes it.
...
| Prospective SPADE-as-method study, 92,400 rows | `results/final-spade-certificate.json`, `results/final-spade-regret-pareto.json` |
