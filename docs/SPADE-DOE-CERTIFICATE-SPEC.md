...
> **HISTORICAL ONLY.** The outputs originally associated with this specification are
> quarantined in `results/historical-dc-constant-yvar/`. Their DoE path reconstructed
> one constant variance from the response instead of using the evaluator's per-well
> variance, and the analyser was committed after partial results existed. They are
> retained for auditability but are **not confirmatory evidence**. Do not place new
> outputs in `results/dc-*.json`; the variance-corrected fresh-seed protocol is
> `docs/SPADE-DOE-CERTIFICATE-CORRECTION-SPEC.md` and is adjudicated separately.
