...
- [x] **METH-01:** SPADE executes exactly 48 evaluations with a scrambled Sobol opening
  selected from 32, 40, or 44 and complete q=4 adaptive batches.
- [x] **METH-02:** Adaptive batches use one objective each: qLogNEI, global IVR, or
  boundary-weighted IVR; raw objective scores are never scalarized together.
- [x] **METH-03:** The validity gate uses only current model diagnostics and falls back to
  global IVR when a stable non-empty boundary is unavailable.
- [x] **METH-04:** All arms use the same learned-noise GP family, fit policy, candidate
  menu, Rule P terminal selector, and scoring functions.

## Reliability map and certificate

- [x] **CERT-01:** The target is `P(Y_new >= tau | f) >= gamma`; changing gamma changes
  the target draws and certificate.
- [x] **CERT-02:** Threshold construction is sealed, arm-independent, and passes only the
  numeric tau to each method.
- [x] **CERT-03:** Certificate construction uses 4,096 draws split 2,048/2,048 and records
  model-conditional cross-fit containment only as a diagnostic.
- [x] **CERT-04:** Confirmatory validity uses empirical truth containment across campaigns;
  empty certificates count only against answer rate.
- [x] **CERT-05:** PASS requires one-sided exact lower bounds above registered answer-rate
...
- [x] **EVAL-02:** Four new randomized d=6 generator families provide independent
...
- [x] **REP-01:** All random streams are statelessly derived, independently named, and
...
- [x] **REP-03:** Release validation fails on missing/duplicate keys, dirty source,
  mismatched hashes, budget/rule mixing, invalid denominators, and premature lockbox use.
- [x] **REP-04:** Historical replay limitations are disclosed; new evidence does not rely
