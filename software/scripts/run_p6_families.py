"""P6 · family coverage — the design space off the Hill oracle, at `tau_q`.
    ...
    OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1 \
      .venv/bin/python -u software/scripts/run_p6_families.py --family levy --dim 6 --sigma 0.25
...
`research/results/p4-coord.json` is committed with 1,400 rows and no AUPRC because the brief that
produced it predated F2b, and recovering it means re-running every campaign.
`true_frac_above_tau` is the same class of omission -- it is why `research/results/versionb.json`
...
OUT = ROOT / "research" / "results" / "p6-families.json"
TAU_TABLE = ROOT / "research/results/p5-tau-quantile.json"
GATE_Q42 = ROOT / "research" / "results" / "q42-families.json"
GATE_D20 = ROOT / "research" / "results" / "d20-rescore.json"
GATE_Q59 = ROOT / "research" / "results" / "q59-hartmann-no-screen.json"
...
#:
#:   ``research/results/k6-designspace.json``         worst 2.220446049250313e-16 = 1.00 ULP
#:   ``research/results/k6-designspace-spread.json``  worst 3.3306690738754696e-16 = 1.50 ULP
#:
...
#: ``software/tests/test_p6_families.py``: zero violations in 60k configurations (worst ratio
...
#: ``software/tests/test_p2_versionb_gamma.py``, and it is written into provenance. It is simply
...
class Tau:
    """One registered threshold, exactly as `research/results/p5-tau-quantile.json` committed it."""
...
def score_campaign(rec, orc, grid, truth, active, taus, arm_label=None) -> list[dict]:
    ...
    for t in taus:
        for gamma in GAMMAS:
            ...
            row = {**base, "p": t.p, "tau_source": "research/results/p5-tau-quantile.json",
...
CENSUS_OUT = ROOT / "research" / "results" / "p6-ceiling-census.json"
    ...
    for sigma in (0.25, 0.10):
        for gamma in GAMMAS:
            ...
            for (family, dim, p), taus in sorted(cells.items()):
                ...
                # NOT changed: `research/results/p6-ceiling-census.json` is committed against its
...
def ckpt_path(family: str, dim: int, sigma: float) -> Path:
    ...
    **This box SIGKILLs workers.** Three runs have died with `BrokenProcessPool` --
    ...
    `research/results/p6-families.json` is written **once, whole, by `--merge`**. A half-finished
    run therefore cannot be mistaken for a finished result: the result file does not exist
    until someone merges.
    """
    return ROOT / "research" / "results" / f"p6-{family}-d{dim}-s{sigma:g}.ckpt.jsonl"
...
def merge() -> None:
    """Assemble every checkpoint into `research/results/p6-families.json`, once, whole."""
    entries, files = [], sorted((ROOT / "research" / "results").glob("p6-*.ckpt.jsonl"))
    for f in files:
...
def main() -> None:
    ...
    ap.add_argument("--merge", action="store_true",
                    help="assemble every checkpoint into research/results/p6-families.json")
    ...
    print(f"  research/results/p6-families.json is NOT written here. Run --merge when cells finish.")
