"""TT adjudicator. Implements publication/manuscript/PROTOCOLS.md (TT) verbatim.
...
def main():
    ...
    paths = (sorted(glob.glob(a.glob)) if a.glob else
             [str(ROOT / "research" / "results" / "mechanism" / f"tt-{f}.json") for f in FAMILIES
              if (ROOT / "research" / "results" / "mechanism" / f"tt-{f}.json").exists()])
    cells, regret = load(paths)
    if not cells:
        print("no TT rows yet"); return
    seeds = sorted({k[2] for k in cells})
    fams = sorted({k[1] for k in cells})
    print("=== TT adjudication — publication/manuscript/PROTOCOLS.md (TT) ===")
