"""Stages 1-3 of the Hall/Ogle digitization plan: raw JSON -> canonical CSVs.
    ...
    python software/scripts/build_published_dataset.py

Produces `research/data/published/hall_ogle_2025_stage{1,2}.csv`, the analysis-ready dataset
`LookupEvaluator` consumes. Deterministic: same inputs, same bytes out, so the
round-trip test in `software/tests/test_published_dataset.py` is meaningful.
...
* **The second extraction was not lost.** It is at `research/data/external/extraction_a/`,
...
sys.path.insert(0, str(ROOT / "software" / "src"))      # runnable without PYTHONPATH, as the
...
def main() -> None:
    for stage in ("stage1", "stage2"):
        path = ROOT / "research" / "data" / "published" / f"hall_ogle_2025_{stage}.csv"
