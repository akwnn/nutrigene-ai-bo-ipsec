...
`research/results/k6-designspace-spread.json`. That gate is **full width**: every column the
...
OUT_DEFAULT = ROOT / "research" / "results" / "p2-versionb-gamma.json"

#: Read-only. The committed gate target for `plate1_only`, and the ONLY gate P2 has.
SPREAD = ROOT / "research" / "results" / "k6-designspace-spread.json"
#: Read-only. The committed Version B columns, used for the determinism block only.
VERSIONB = ROOT / "research" / "results" / "versionb.json"
VERSIONB_PRED = ROOT / "research" / "results" / "versionb-predictive.json"
...
NEVER_OVERWRITE = (VERSIONB, VERSIONB_PRED, SPREAD,
                   ROOT / "research" / "results" / "k6-designspace.json")
...
_VB = importlib.util.module_from_spec(
    importlib.util.spec_from_file_location("_run_versionb",
                                           ROOT / "software" / "scripts" / "run_versionb.py"))
...
def work_path_for(out: Path) -> Path:
    ...
    `.gitignore:199` carries ``!research/results/p2-versionb-gamma.json``, so the deliverable
...
def is_git_tracked(path: Path) -> bool:
    """Is this path already under version control?

    The overwrite guard is "is it tracked", not a hand-maintained list of filenames --
    a list is a thing someone has to remember to extend, and `NEVER_OVERWRITE` already
    proved that by needing four entries. Anything git tracks is a committed result.
    """
    try:
...
def resumable_rows(path: Path) -> list[dict]:
    """Rows already on disk, or `[]`. **Refuses to resume across a schema change.**

    Amendment F added the error volumes and AUPRC. A file written before them holds rows
    that cannot be told apart from current ones except by which key they belong to, and
    a per-cell table would average the two populations without showing that it had. So
    a missing column is a stop, not a backfill.
    """
    if not path.exists():
        return []
    ...
    if not rows:
        return []
    missing = [k for k in SCHEMA_REQUIRED if k not in rows[0]]
    if missing:
        raise SystemExit(
            f"{path} was written under a different schema -- it is missing {missing}. "
            f"Resuming would mix two populations of rows in one file. Re-run with "
            f"--no-resume, or point --out somewhere new.")
    return rows
...
def gate_note() -> str:
    ...
    return (f"{GATED_ARM} IS `lhs` at {BUDGET} wells and is gated full-width against "
            f"research/results/k6-designspace-spread.json at |delta| = {GATE_TOL}; it may never be "
...
def _memoised_containment():
    ...
    **This does not change the estimator and it is not an approximation.**
    ...
    -- and ``software/tests/test_p2_versionb_gamma.py`` asserts every column of a real campaign
    ...
    for gamma in gammas:
        for tf in tau_fracs:
            ...
            rows.append({**base, "gamma": gamma, "tau_frac": tf, "tau": tau,
                         ...
                         "vol_pred": vol_p, "vol_latent": vol_l,
                         ...
                         # Amendment F2b: AUPRC beside AUC, primary under imbalance.
                         "auprc_pred": auprc(p_pred, truth, tau),
                         "auprc_latent": auprc(p_lat, truth, tau),
...
def main() -> None:
    ...
    if is_git_tracked(out):
        raise SystemExit(f"refusing to write {_rel(out)} -- it is git-tracked, so it is "
                         f"already a committed result. Pass a different --out.")
