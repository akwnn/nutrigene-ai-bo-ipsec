"""DC adjudicator. Implements publication/manuscript/PROTOCOLS.md (DC) verbatim.
...
def main():
    ...
    paths = (sorted(glob.glob(a.glob)) if a.glob else
             [str(ROOT / "research" / "results" / "comparisons" / f"dc-{f}.json") for f in FAMILIES
              if (ROOT / "research" / "results" / "comparisons" / f"dc-{f}.json").exists()])
    ...
    print("=== DC adjudication — publication/manuscript/PROTOCOLS.md (DC) ===")
