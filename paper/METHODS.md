...
`data/lab/derived/candidate_campaign_coating_flow.csv`, linked to raw FCS files, manual-role
overlays, and checksums. The published support uses the canonical Hall/Ogle stage-1 and
stage-2 extractions under `data/published/`. Mean-marginalized covariance propagates
...
`paper-evidence/reproduction-map.md`. `scripts/verify_conclusions.py` recomputes the guarded
